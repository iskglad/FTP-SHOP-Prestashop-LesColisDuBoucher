...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#delivery_option, #date_delivery, #hour_delivery_from, #hour_delivery_to, #carrier_recycled_package,#order_gift,#gift_message').change(function() {
    ...
    function displayPrixKilo(id) {
        ...
        $('#prix_kilo').html(stock[id_product][id_product_attribute]);
    }
    ...
    function updateDeliveryHour(){
        var html_from = '';
        var html_to = '';
        var digits = "00h00";

        for (var i = 8; i <= 22; i++) {
            digits = ("0" + i).slice(-2) + 'h00';
            html_from += '<option value="'+ digits +'" '+ (($('#hour_delivery_from').val() == digits) ? 'selected="selected"' : '') +'>'+ digits + '</option>';
            html_to += '<option value="'+ digits +'" '+ (($('#hour_delivery_to').val() == digits) ? 'selected="selected"' : '') +'>'+ digits + '</option>';
        }

        $('#hour_delivery_from').html(html_from);
        $('#hour_delivery_to').html(html_to);
    }
	...
	function updateDeliveryOption()
	{
		$.ajax({
			...
			data : {
                ...
                hour_delivery_from: $('#hour_delivery_from option:selected').val(),
                hour_delivery_to: $('#hour_delivery_to option:selected').val(),
	...
	function updateAddressesList(addresses, id_address_delivery, id_address_invoice)
	{
		...
		$.each(addresses, function() {
            ...
            //add invoice for client addresses only
            if (this.id_carrier_relay == 0)
...
<fieldset id="products_part" style="display:none;"><legend><img src="../img/t/AdminCatalog.gif" />{l s='Cart'}</legend>
	<div>
		...
		<div id="products_found">
            ...
            <p><label for="poids">{l s='Poids réel:'}</label><input type="text" name="poids" id="poids" value="1" />&nbsp;<b>{l s='Prix au kilo:'}</b>&nbsp;<span id="prix_kilo"></span></p>
...
<fieldset id="carriers_part" style="display:none;">
	...
	<div id="carrier_form">
		<div>
            ...
            <p>
                <label for="hour_delivery">{l s='Date de livraison:'}</label>
                <input class='datepicker date' id="date_delivery" name="date_delivery"/>
            </p>
            <p>
                <label for="hour_delivery">{l s='Horraires:'}</label>
                Entre
                <select id="hour_delivery_from" name="hour_delivery">
                </select>
                et
                <select id="hour_delivery_to" name="hour_delivery">
                </select>
            </p>
		</div>
		<div id="float:left;">

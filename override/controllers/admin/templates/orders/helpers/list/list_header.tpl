				...
				<td style="vertical-align: bottom;">
					...
					<span style="float: right;">
						...
						<input type="submit" id="submitFilterButton{$table}" name="submitFilter" value="{l s='Filter'}" class="button" />
                                        ...
                                        {if $table == 'order'}      
                                        <span class="date{$table}">
                                            {foreach $fields_display AS $key => $params}
                                                {if $params.type == 'date' || $params.type == 'datetime'}
                                                    &nbsp;{if strstr($params.name_date,'date_add')}{l s='Day order'}{elseif strstr($params.name_date,'date_delivery')}{l s='Day delivery'}{/if} <input type="text" class="filter datepicker" id="{$params.id_date}_2" name="{$params.name_date}[2]" value="{if isset($params.value.2)}{$params.value.2}{/if}"{if isset($params.width)} style="width:70px"{/if}/>
{/if}
{/foreach}
<input type="submit" id="submitFilterButtonorderbydate" name="submitFilterorderbydate" value="Filtre" class="button">
</span>
<span>
    <a id="desc-order-export" class="button" href="index.php?controller=AdminOrders&amp;exportorderbydate&amp;token={$token}" title="Exporter" target="_black">Exporter</a>
</span>

<span>
    <a id="desc-order-export" class="button" href="index.php?controller=AdminOrders&amp;exportorderbydatetocsv&amp;token={$token}" title="Exporter" target="_black">Exporter CSV</a>
</span>
                                        {/if}

...
class AdminRecipeCategoriesControllerCore extends AdminController
{
	/** @var object CMSCategory() instance for navigation*/
	protected $recipe_category;

	protected $position_identifier = 'id_recipe_category_to_move';

	public function __construct()
	{
		$this->table = 'recipe_category';
		$this->className = 'RecipeCategory';
		$this->lang = true;
		...
		'id_recipe_category' => array('title' => $this->l('ID'), 'align' => 'center', 'width' => 30),
		...
		{
			$this->redirect_after = '?controller=AdminRecipeContent&token='.Tools::getAdminTokenLite('AdminRecipeContent');
			$this->redirect();
		}

		$this->recipe_category = AdminRecipeContentController::getCurrentCMSCategory();
		$this->_filter = 'AND `id_parent` = '.(int)$this->recipe_category->id;
	...
	{
		$this->initToolbar();
		$this->toolbar_title = $this->l('Recipe Category');
	...
	public function postProcess()
	{
		...
		{
			$this->action = 'save';
			if ($id_recipe_category = (int)Tools::getValue('id_recipe_category'))
			{
				$this->id_object = $id_recipe_category;
				if (!RecipeCategory::checkBeforeMove($id_recipe_category, (int)Tools::getValue('id_parent')))
				{
		...
		elseif (Tools::isSubmit('statusrecipe_category') && Tools::getValue($this->identifier))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				if (Validate::isLoadedObject($object = $this->loadObject()))
				{
					...
					{
						$identifier = ((int)$object->id_parent ? '&id_recipe_category='.(int)$object->id_parent : '');
		...
		{
			$object = new RecipeCategory((int)Tools::getValue($this->identifier, Tools::getValue('id_recipe_category_to_move', 1)));
			if ($this->tabAccess['edit'] !== '1')
				$this->errors[] = Tools::displayError('You do not have permission to edit here.');
			...
			{
				$identifier = '';
				if ($id_category = (int)Tools::getValue($this->identifier, Tools::getValue('id_recipe_category_parent', 1)))
		...
		elseif (Tools::getValue('submitDel'.$this->table) || Tools::getValue('submitBulkdelete'.$this->table))
		{
			if ($this->tabAccess['delete'] === '1')
			{
				if (Tools::isSubmit($this->table.'Box'))
				{
					...
					$result = true;
					$result = $recipe_category->deleteSelection(Tools::getValue($this->table.'Box'));
					if ($result)
					{
						$recipe_category->cleanPositions((int)Tools::getValue('id_recipe_category'));
						$token = Tools::getAdminTokenLite('AdminRecipeContent');
	...
	public function renderForm()
	{
		...
		$html_categories = RecipeCategory::recurseCMSCategory($categories, $categories[0][1], 1, $this->getFieldValue($this->object, 'id_parent'), 1);

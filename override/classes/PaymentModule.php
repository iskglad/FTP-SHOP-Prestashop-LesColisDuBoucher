        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
        {
            ...
            foreach ($package_list as $id_address => $packageByAddress)
                foreach ($packageByAddress as $id_package => $package)
                {
                    ...
                    //Line changed because $id_address is valid address only.
                    //Sometimes we use carrier relay address.
                    //These are not valid, so $id_address is replaced by first valid address found
                    //instead of using carrier address
                    ...
                    */
                    ...
                    $order->id_order_to_adjust = $this->context->cart->id_order_to_adjust;
                    $order->message = $this->context->cart->message;


                    //delete adjustment infos from session
            ...
            foreach ($order_detail_list as $key => $order_detail)
            {
                ...
                if (!$order_creation_failed & isset($order->id))
                {
                    ...
                    {
                        //Get Label
                        $combinaison = array('attributes' => Product::getProductAttributes($product['id_product_attribute'])); //init
                        $product['label'] = Product::getCombinationLabelName($combinaison);
                        ...
                        if (isset($customized_datas[$product['id_product']][$product['id_product_attribute']]))
                        {
                            ...
                            $products_list .=
								...
								<td style="padding: 0.6em 0.4em;width: 30%;"><strong>'.$product['name'].(isset($product['attributes']) ? ' - '.ucfirst($product['label']) : '').' - '.Tools::displayError('Customized').(!empty($customization_text) ? ' - '.$customization_text : '').'</strong></td>
                        ...
                        if (!$customization_quantity || (int)$product['cart_quantity'] > $customization_quantity)
                            $products_list .=
                                '<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
								<td style="padding: 0.6em 0.4em;width: 30%;"><strong>'.$product['name'].(isset($product['attributes']) ? ' - '.ucfirst($product['label']) : '').'</strong></td>
								<td style="padding: 0.6em 0.4em; width: 20%;">'.Tools::displayPrice(Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt, $this->context->currency, false).'</td>
                    ...
                    {
                        echo 'foreach-'.$cart_rule['obj']->name;
                        ...
                        if ($cart_rule['obj']->name == RULE_NAME_FREE_SHIPPING)
                            $isFreeShipping = true;
                        ...
                        */
                        ...
                        //if FREE_SHIPPING rule dont display it
                        if ($cart_rule['obj']->name != RULE_NAME_FREE_SHIPPING){
                            $cart_rules_list .= '
						    <tr>
							    <td colspan="3" style="padding:0.6em 0.4em;text-align:right">'.Tools::displayError('Voucher name:').' '.$cart_rule['obj']->name.'</td>
                            ...
                            $discounts_html .= '
                                <tr style="text-align: right;">
                                    <td>&nbsp;</td>
                                    <td style="background-color: #ebecee; padding: 0.6em 0.4em;" colspan="2">'.ucfirst($cart_rule['obj']->name).'</td>
                                    <td style="background-color: #ebecee; padding: 0.6em 0.4em;">'.($values['tax_incl'] != 0.00 ? '-' : '').Tools::displayPrice($values['tax_incl'], $this->context->currency, false).'</td>
                                </tr>
                            ';
                        }
                    }
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
                    {
                        ...
                        $total_discount = Tools::displayPrice($order->total_discounts, $this->context->currency, false);
                        if ($order->total_discounts > 0)
                            $total_discount_html = '<tr style="text-align: right;">
                                                        <td>&nbsp;</td>
                                                        <td style="background-color: #ebecee; padding: 0.6em 0.4em;" colspan="2">Bons d&#039;achat et r&eacute;ductions</td>
                                                        <td style="background-color: #ebecee; padding: 0.6em 0.4em;">'.$total_discount.'</td>
                                                    </tr>';
                        else
                            $total_discount_html = '';

                        if ($isFreeShipping){
                            $total_shipping = 'Offert';
                            //Update order infos
                            $order->free_shipping_discount = 1;
                            $order->save();
                        }
                        else
                        ...
                        $data = array(
                            ...
                            '{total_products}' => Tools::displayPrice($order->total_products_wt),//Tools::displayPrice($order->total_paid - $order->total_shipping - $order->total_wrapping + $order->total_discounts, $this->context->currency, false),
                            '{total_discounts}' => Tools::displayPrice($order->total_discounts, $this->context->currency, false),
                            '{total_shipping}' => $total_shipping,
                            '{total_wrapping}' => Tools::displayPrice($order->total_wrapping, $this->context->currency, false),
                            '{date_delivery}' => Tools::displayDate($this->context->cart->date_delivery, (int)$order->id_lang),
                            '{hour_delivery}' => $this->context->cart->hour_delivery,
                            '{total_discount_html}' => $total_discount_html,

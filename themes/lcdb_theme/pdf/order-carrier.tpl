...
<!-- 					<div id="delivery-relay" class="hidden">
												...
												{foreach $option.carrier_list as $carrier}
													<p>
														<label class="radio" for="delivery_option_{$id_address}_{$option@index}">
                                                                                                                    <input class="choose-relay_" idaddress="{$id_address}" type="radio" name="delivery" value="{$carrier.instance->id}" onchange="updateExtraCarrier('{$key}', {$id_address});" id="delivery_option_{$id_address}_{$option@index}"/><!-- {if $carrier.instance->id_reference == $ID_RELAY_CARRIER}{/if}-->
			...
			<div id="continue-shopping">
				...
				{if $minimum_order < $cart->getOrderTotal(true, $order_total_flag_without) && $id_zone_avalibal != 0}<!--$cp == 75-->
